...
public class PlayerSyncController : MonoBehaviour
{
    ...
    void Update()
    {
        if (pv.IsMine) // xr origin과 싱크 오브젝트 동기화(포톤으로 넘겨주기 위한)
        {
            ...
            SyncTransform(head, headRig);                // headRig = xr origin의 main camera
            SyncTransform(leftHand, leftHandRig);        // leftHandRig = xr origin의 left controller
            SyncTransform(rightHand, rightHandRig);      // rightHandRig = xr origin의 right controller
        }
    }
