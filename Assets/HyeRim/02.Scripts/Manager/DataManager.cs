...
namespace NHR
{
    public class DataManager
    {
        ...
        {
            TextAsset asset = Resources.Load<TextAsset>("Datas/missionData");

            var json = asset.text;
            Debug.LogFormat("<color=red>mission data load:{0}</color>", json);

            //역직렬화
            MissionData[] datas = JsonConvert.DeserializeObject<MissionData[]>(json);

            this.dicMissionData = datas.ToDictionary(x => x.id);
            Debug.LogFormat("mission data loaded : {0}", this.dicMissionData);
        }
